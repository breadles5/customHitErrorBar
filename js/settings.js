...
export const updateSettings = (message) => {
  ...
  if (
    Object.prototype.hasOwnProperty.call(message, "showSD") &&
    oldSettings.showSD !== message.showSD
  ) {
    updateVisibility();
  }
};
...
const updateCSSLayout = () => {
  ...
  if (windowHeight !== lastWindowHeight) {
    ...
    const clampedHeight = Math.max(
      0,
      Math.min(100, settings.timingWindowHeight),
    );
...
const updateCSSColors = () => {
  root.style.setProperty(
    "--timing-windows-opacity",
    settings.TimingWindowOpacity,
  );
